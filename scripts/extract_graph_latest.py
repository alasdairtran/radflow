...
def pages_from(f, done_ids):
    ...
    for line in f:
        ...
        if not isinstance(line, str) and b'<' not in line and ignore_page:
            continue
        elif isinstance(line, str) and '<' not in line and ignore_page:
            continue
...
def process_path(path, result_dir, i):
    ...
    # Assume we're continuing from existing job
    ...
    # print(filename[34:], len(done_ids))

    with BZ2File(path) as fin:
        with open(result_path, 'a') as fout:
            for page in tqdm(pages_from(fin, done_ids), position=i, desc=filename[34:]):
    ...
    with open(f'{result_dir}/archives4.jsonl', 'a') as f:
        out = {
            '_id': filename,
            'elapsed': time.time() - start_time,
...
def extract_wiki_graph(host, dump_dir, result_dir):
    # client = MongoClient(host=host, port=27017)
    # db = client.wiki
    ...
    # Keep only paths which we have not finished procesing
    result_paths = ['results/archives1.jsonl',
                    'results/archives2.jsonl',
                    'results/archives3.jsonl']
    done_dumps = set()
    ...
    print(f'Done processing {len(done_dumps)} dump files')

    paths = list(filter(lambda p: os.path.basename(p)
                        not in done_dumps, paths))

    print(f'Will process {len(paths)} more dump files')

    with Parallel(n_jobs=len(paths), backend='loky') as parallel:
        parallel(delayed(process_path)(path, result_dir, i)
