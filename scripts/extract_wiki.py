...
def string_replace(args):
    params = functionParams(
        args, ('source', 'pattern', 'replace', 'count', 'plain'))
    source = params.get('source', '')
...
def callParserFunction(functionName, args, extractor):
    ...
    try:
        ...
        if functionName == '#invoke':
            ...
            logging.debug('%*s#invoke %s %s %s',
                          extractor.frame.depth, '', module, fun, args[2:])
            ...
            logging.debug('%*s<#invoke %s %s %s',
                          extractor.frame.depth, '', module, fun, ret)
            return ret
        ...
        # handle lists
            ...
            if line:  # FIXME: n is '"'
                if options.keepLists:
                    ...
                    listCount[i - 1] += 1
                    bullet = 'BULLET::::%d. ' % listCount[i -
                                                          1] if n == '#' else 'BULLET::::- '
                    page.append('{0:{1}s}'.format(
                        bullet, len(listLevel)) + line)
                elif options.toHTML:
...
def page_history_from(input_lines):
    # The output is a dictionary that maps a timestamp string,
    # e.g. 2002-02-02T11:12:48Z, to a revision Dict. This will make it easier
    # to sort by the timestamp key later on.
    ...
    for line in input_lines:
        ...
        # If the line doesn't contain any XML tags
        if '<' not in line:
            # Case 1: If we're inside a text block
            # Case 2: If we're not inside a text block, e.g. inside another
            # tag block, we simply don't care
            pass

        # If the line contains '<' this could mean a tag, but it could also
        # be just the normal character '<' in a text block.
        m = tagRE.search(line)
        if not m:
            continue
        tag = m.group(2)

        # Each Wikipedia page is wrapped around the <page> tag.
        # Thus if we see this tag, it's a start of a new page.
        if tag == 'page':
            assert not history
            in_page = True
        ...
        # Extract revision tags
        elif in_page and in_revision:
            if tag == 'text':
                # ignore self closing <tag ... /> tags
                ...
                # Extract only the text, ignore the tag itself
            ...
            # We see a closing </text> tag, then we've done with that revision
            elif tag == '/text':
                if m.group(1):
                    article.append(m.group(1))
                in_text = False
    ...
    for line in input:
        ...
        if '<' not in line:  # faster than doing re.search()
            ...
            # Check to see if we're inside a paragraph text
            if inText:
                page.append(line)
                # extract categories
        ...
        # One level below <page>, we get the following tags:
        # <id> Unlike the title, the Page ID does not change
        #     when a page is moved, which makes it a stable reference.
        elif in_page and not in_revision:
            if tag == 'id':
                assert page_id == None
                page_id = m.group(3)
        ...
        # <title> Current title of article
        elif tag == 'title':
            title = m.group(3)

        # <ns> of 0 are (Main/Article). See
        # https://en.wikipedia.org/wiki/Wikipedia:Namespace
        elif tag == 'ns':
            ns = m.group(3)

        # Where this page is a redirection
        elif tag == 'redirect':
            redirect = True

        # <text> indicates the starting text
        elif tag == 'text':
            # ignore self closing <tag ... /> tags
            if m.lastindex == 3 and line[m.start(3)-2] == '/':
                # <text xml:space="preserve" />
        ...
        # We see a closing </text> tag
        elif tag == '/text':
            if m.group(1):
                page.append(m.group(1))
            inText = False

        # Assume that if we're inside <text>, there is no other nested tag.
        elif inText:
            page.append(line)

        # We've reached the end of the page
        elif tag == '/page':
            if page_id != last_id and not redirect:
                yield (page_id, revid, title, ns, catSet, page, lines)
                last_id = page_id
                ns = '0'
            page_id = None
            revid = None
    ...
    if options.expand_templates:
        ...
        if template_file:
            if os.path.exists(template_file):
                logging.info(
                    "Loading template definitions from: %s", template_file)
                # can't use with here:
        ...
        logging.info("Loaded %d templates in %.1fs", len(
            options.templates), template_load_elapsed)
        ...
        with open(filter_category) as f:
            ...
            for line in f.readlines():
                try:
                    ...
                    error_cnt += 1
                    print(u"Category not in utf8, ignored. error cnt %d:\t%s" %
                          (error_cnt, e))
                    print(line)
