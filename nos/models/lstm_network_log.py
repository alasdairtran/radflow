...
@Model.register('time_series_lstm_network_log')
class TimeSeriesLSTMNetworkLog(BaseModel):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 data_dir: str,
    ...
    def _forward(self, series):
        ...
        # Take the difference
        log_series = torch.log1p(series)
        targets = log_series[:, 1:]
        inputs = log_series[:, :-1]
    ...
    def _populate_cache(self, splits):
        ...
        for day in range(self.n_days):
            ...
            for i in range(0, len(keys), batch_size):
                ...
                log_pred = self.fc(X_full).squeeze(2).squeeze(1)
                # log_pred.shape == [batch_size]
