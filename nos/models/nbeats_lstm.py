...
@Model.register('nbeats_lstm')
class NBEATSLSTM(BaseModel):
    def __init__(self,
                 vocab: Vocabulary,
                 data_dir: str,
        ...
        for i in range(n_stacks):
            self.layers.append(LSTMLayer(hidden_size, dropout, agg))
...
class LSTMLayer(nn.Module):
    def __init__(self, hidden_size, dropout, agg):
        super().__init__()
    ...
    def forward(self, X, yn, masks):
        ...
        bn = None
        if self.agg:
            X, bn = self.aggregate(X, yn, masks)
            # X.shape == [batch_size, seq_len, hidden_size]
    ...
    def aggregate(self, X, yn, masks):
        ...
        bn = self.out_b(F.gelu(self.proj_b(yn))).squeeze(-1)
        # bn.shape == [batch_size, n_neighs, seq_len]
