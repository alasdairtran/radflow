...
@Model.register('time_series_lstm_network_tags')
class TimeSeriesLSTMNetworkTags(BaseModel):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 data_dir: str,
        ...
        self.embed_tags = GehringLinear(len(TAGS), 15)
        ...
        tag_path = os.path.join(data_dir, 'vevo_tags.json')
        with open(tag_path) as f:
            self.tags = json.load(f, object_pairs_hook=keystoint)
    ...
    def _initialize_series(self):
        ...
        for k, tags in self.tags.items():
            tag_tensor = p.new_zeros(len(TAGS))
            for tag in tags:
                tag_tensor[TAG_DICT[tag]] = 1
            self.tags[k] = tag_tensor

    def _forward(self, series, tags):
        # series.shape == [batch_size, seq_len]
        # tags.shape == [batch_size, n_tags]
        ...
        tags = self.embed_tags(tags)
        tags = tags.unsqueeze(1)
        # tags.shape == [batch_size, 1, tag_embed]

        tags = tags.expand(tags.shape[0], X.shape[1], tags.shape[2])
        # tags.shape == [batch_size, seq_len - 1, tag_embed]

        X = torch.cat([X, tags], dim=-1)
        # X.shape == [batch_size, seq_len - 1, 1 + tag_embed]
    ...
    def _forward_full(self, series, tags):
        # series.shape == [batch_size, seq_len]
        # tags.shape == [batch_size, n_tags]
    ...
    def _get_neighbour_embeds(self, X, keys, day=None):
        ...
        for key in keys:
            ...
            for s in sources:
                ...
                tag_list.append(self.tags[s])

        sources = torch.stack(source_list, dim=0)
        # sources.shape == [batch_size * n_neighbors, seq_len]

        tags = torch.stack(tag_list, dim=0)
        # tags.shape == [batch_size, n_tags]

        X_neighbors = self._forward_full(sources, tags)
    ...
    def forward(self, keys, splits) -> Dict[str, Any]:
        ...
        X, targets = self._forward(series, tags)
    ...
    def _populate_cache(self, splits):
        ...
        for day in range(self.n_days):
            ...
            for i in range(0, len(keys), batch_size):
                ...
                hidden = self._forward_full(series, tags)
