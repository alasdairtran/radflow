...
def to_bit_vector(x):
    return [float(c) for c in np.binary_repr(int(x), width=24)]


def bool2int(x):
    # From https://stackoverflow.com/a/15505648
    y = 0
...
@Model.register('nbeats_transformer_bit')
class NBEATSTransformerBit(BaseModel):
    def __init__(self,
                 vocab: Vocabulary,
                 data_dir: str,
        ...
        self.upsample = GehringLinear(24, hidden_size)
    ...
    def _initialize_series(self):
        ...
        for k, v in self.series.items():
            ...
            s = [to_bit_vector(x) for x in self.series[k]]
            self.series[k] = p.new_tensor(s)
    ...
    def _get_neighbours(self, keys, split, start):
        ...
        sources = self._float.new_zeros(
            (len(keys), self.max_neighbours, self.total_length, 24))
    ...
    def forward(self, keys, splits) -> Dict[str, Any]:
        ...
        S = series
        # series.shape == [batch_size, total_length, embed_dim]

        n_series, masks = self._get_neighbours(keys, split, start)
        Sn = n_series
        ...
        Xn = Sn[:, :, :-1]
        yn = Sn[:, :, 1:]
        # S.shape == [batch_size, total_len, embed_dim]
        # Sn.shape == [batch_size, n_neighs, total_len, embed_dim]
        # masks.shape == [batch_size, n_neighs]
        # yn.shape == [batch_size, n_neighs, seq_len, embed_dim]

        forecast = self._forward(X, Xn, yn, masks)
        forecast = torch.sigmoid(forecast)
        # forecast.shape == [seq_len, batch_size, embed_dim]

        targets = y.transpose(0, 1)
        # targets.shape == [seq_len, batch_size, embed_dim]
        ...
        if splits[0] in ['test']:
            ...
            targets = torch.zeros(B, self.forecast_length)
            preds = torch.zeros(B, self.forecast_length)

            for i in range(self.forecast_length):
                forecast = self._forward(X, Xn, None, masks)[-1]
                # forecast.shape == [batch_size, embed_dim]
                forecast = torch.sigmoid(forecast)
                forecast = forecast.round()
                forecast_list = forecast.long().cpu().numpy().tolist()
                forecast_list = [bool2int(f[::-1]) for f in forecast_list]
                preds[:, i] = preds.new_tensor(forecast_list)

                offset = S.shape[1]-self.forecast_length + i
                target_list = S[:, offset].long().cpu().numpy().tolist()
                target_list = [bool2int(f[::-1]) for f in target_list]
                targets[:, i] = targets.new_tensor(target_list)
    ...
    def _forward(self, X, Xn, yn, masks):
        ...
        # Xn.shape == [seq_len, batch_size, n_neighs, embed_dim]
        ...
        masks = masks.unsqueeze(0).expand(Xn.shape[0], -1, -1)
        # masks.shape == [seq_len, batch_size, n_neighs]

        Xn = self.upsample(Xn)

        T, B, E = X.shape
        forecast = X.new_zeros(T, B, 24)
...
class TBEATLayer(nn.Module):
    def __init__(self, hidden_size, dropout, n_heads, forecast_len):
        ...
        self.final_out = GehringLinear(hidden_size, 24)
    ...
    def forward(self, X):
        ...
        f = self.final_out(X)
        # out.shape == [seq_len, batch_size, embed_dim]
