...
@Model.register('time_series_lstm_network_weighted')
class TimeSeriesLSTMNetworkWeighted(BaseModel):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 data_dir: str,
        ...
        self.fc = GehringLinear(self.hidden_size + 1, 1)
    ...
    def _get_neighbour_embeds(self, X, keys, day=None):
        ...
        sources[sources == 0] = 1
        diff = sources[:, -1] / sources[:, -2]
        # sources.shape == [batch_size * n_neighbors]
        ...
        for n_neighbors, X_i in zip(neighbor_lens, X):
            if n_neighbors == 0:
                diff_i = X_i.new_zeros(1, 1)
            else:
                diff_i = diff[cursor:cursor + n_neighbors]
                views_i = sources[cursor:cursor + n_neighbors, -1]
                # diff_i.shape == [n_neighbors]
                # views_i.shape == [n_neighbors]

                weights_i = views_i / views_i.sum()
                diff_i = (diff_i * weights_i).sum().unsqueeze(0).unsqueeze(1)

            X_full = torch.cat([X_i, diff_i], dim=-1)
            X_full_list.append(X_full)

            cursor += n_neighbors

        X_full = torch.stack(X_full_list, dim=0)
        # X_full.shape [batch_size, seq_len, hidden_size + 1]
